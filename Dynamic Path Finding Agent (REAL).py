...
# GBFS returns path, visited, frontier_set, count
def gbfs(grid, start, goal, rows, cols, hfn):
    ...
    while frontier:
        ...
        count += 1
        if cur == goal:
            return reconstruct_path(came_from, goal), visited, frontier_set, count
        for nb in get_neighbors(cur, grid, rows, cols):
            if nb not in visited:
                came_from[nb] = cur
                heapq.heappush(frontier, (hfn(nb, goal), nb))
                frontier_set.add(nb)
    return None, visited, frontier_set, count

# Astar returns path, visited, frontier_set, count
...
class App:
    ...
    def run_search(self):
        ...
        path, vis, front, nv = self._algo_fn()(
        ...
        if random.random() < 0.08:
            ...
            if pos not in (self.start, self.goal) and self.grid[r][c] == 0:
                ...
                if pos in self.path[self.agent_idx:]:
                    ...
                    new_path, vis, front, nv = self._algo_fn()(
    ...
    def _draw_panel(self):
        ...
        # Legend now includes frontier
        legend = [
            (C_START,    "Start node"),
            (C_GOAL,     "Goal node"),
            (C_WALL,     "Wall / obstacle"),
            (C_FRONTIER, "Frontier nodes"),
